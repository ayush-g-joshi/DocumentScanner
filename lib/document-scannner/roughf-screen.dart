import 'dart:io';
import 'package:docscanner/document-scannner/image-cropper.dart';
import 'package:flutter/material.dart';
import 'package:file_picker/file_picker.dart';

import '../listview_all_features/list-task.dart';
  ...
  void _pickFile() async {
    ...
    result.files.first.path != null
        ? Navigator.of(context).push(MaterialPageRoute(
            builder: (_) => ChoosePdfFile(
                  pdf_address: result.files.first.path,
                )))
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
          ...
          children:  [
            Center(child: Text("For task number 1--")),
           Center(
             child: ElevatedButton(onPressed: () {
               Navigator.of(context).push(MaterialPageRoute(
                   builder: (_) => ListViewTask()));
             }, child: Text("ListView-Task1")),
           ),

            SizedBox(height: 50,),

            Text("For task 2-please open it from app bar add icon")
          ]),
    );
  }
